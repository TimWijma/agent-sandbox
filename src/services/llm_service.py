...
class LLMService:
    ...
    async def create_plan(self, objective: str) -> Plan:
        ...
        try:
            ...
            input_tokens, output_tokens = self.get_token_count(response.usage)
    ...
    async def execute_plan(self, conversation_id: int, plan: Plan):
        ...
        llm_initial_response = await asyncio.get_event_loop().run_in_executor(
            ...
            lambda: completion(
                model=self.model,
                messages=[
                    {"role": ChatRole.USER.value, "content": msg}
                ],
                temperature=0.2,
        ...
        current_message = StepMessage.model_validate_json(model_response)
        # conversation.messages.append(Message(
        #     id=len(conversation.messages),
        ...
        logger.info(f"Starting plan execution with initial message: {current_message}")

        if self.ui_callback:
            await self.ui_callback(current_message.message)

        # Now, loop through steps until plan is complete
        while not current_message.plan_complete:
            current_step = current_message.step
            ...
            logger.info(f"Proceeding to next step with message: {current_message}")
